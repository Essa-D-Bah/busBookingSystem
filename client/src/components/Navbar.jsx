  ...
  const drawer = (
    <Box onClick={handleDrawerToggle} sx={{ textAlign: "center" }}>
      ...
      <Box display={'flex'} flexDirection={"column"} paddingTop={2}>
        {navItems.map((item) => (
          <NavLink key={item.path} style={({isActive, isPending })=>{
           return{
            ...mobileStyle,
            backgroundColor: isActive ? "#011627" : isPending ? "#f57c00" : "#14213d",
            display:user.role==="client" && (item.label  ==='Dashboard'|| item.label==="Buses"||item.label==="Companies")?'none':"block"
           }
