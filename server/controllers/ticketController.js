  ...
  try {
    ...
    const allTicket = await Promise.all(
      tickets.map(async (ticket) => {
        ...
        const fetchedTrip = await Trip.findOne({ _id: tripId });
        const road = fetchedTrip.startCity + "__" + fetchedTrip.destination;
        const newTicket = { ...ticket._doc, road };
        return newTicket;
      })
    );
    res.json({ allTicket });
...
async function createTicket(req, res) {
  try {
    const { tripId, userId, departureTime, price, busId } = req.body;
